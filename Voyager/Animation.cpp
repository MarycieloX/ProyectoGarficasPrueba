...
void Animation::PlayWalkFPS(Model& weapon, Camera& cam, float dt)
{
	...
	// Gire el arma de nuevo a 190 grados si no está a 190 grados
	...
	if (m_weaponMoveForward)
	{
		...
		// Evita que el arma vaya demasiado lejos en el mundo.
		if (m_defWeaponZOffset <= MAX_Z_OFFSET)
	...
	else
	{
		...
		// Evita que el arma vaya demasiado lejos detrás de la vista.
...
{
	// Devuelve el arma a la posición Z original
